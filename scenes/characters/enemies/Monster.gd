...
onready var nav : Navigation = get_tree().get_root().find_node("Navigation", true, false)
...
func _ready():
	...
	for attack_spawner in aimer.get_children():
		if attack_spawner.has_method("set_bodies_to_exclude"):
			attack_spawner.set_bodies_to_exclude([self])
